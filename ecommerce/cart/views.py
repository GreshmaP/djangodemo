...
def cart_view(request):
    ...
    for i in c:
        total+=i.quantity*i.product.price #calculates sum of each product price*quantity
...
def cart_delete(request,i):
    ...
    p = Product.objects.get(id=i)
    try:
        c=Cart.objects.get(user=u,prduct=p)
        c.delete()
        p.stock+=c.quantity
        p.save()
    except:
...
def orderform(request):
    if(request.method=="POST"):
        ...
        total=int(total*100)
        #print(total)
        client=razorpay.Client(auth=('rzp_test_cJ64rpjTtvKRvz','oYOp8EJVJwEJXGT1WvVRgGjK')) #Creates a client connection
        ...
        #razopay using razorpay client
        #print(response_payment)
        order_id=response_payment['id'] #Retrieves order_id from response
        order_status=response_payment['status']  #Retrieves status from response
        if(order_status=="created"): #if status is created then store oreder_id in Payment and Order_details table
...
def payment_status(request,u):
    ...
    if request.method=="POST":
        ...
        client=razorpay.Client(auth=('rzp_test_cJ64rpjTtvKRvz','oYOp8EJVJwEJXGT1WvVRgGjK'))#To create a razorpay client
        print(client)
        try:
            status=client.utility.verify_payment_signature(param_dict)#To check the authenticity of the razorpay signature
            print(status)
            # To retrieve a particular record in Payment table whose order_id matches the response order id
            ...
            print(p.razorpay_payment_id)
            p.paid=True #changes the paid status to True
            p.save()


            #print(user.username)
            o=Order_details.objects.filter(user=user,order_id=response['razorpay_order_id']) #Retrieves the record in order_details
            #print(o)
            ...
            # After successful payments deletes the items in cart for a particular user
